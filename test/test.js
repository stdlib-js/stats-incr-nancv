...
tape( 'the function throws an error if not provided a number for the mean value', function test( t ) {
	var values;
	var i;
...
tape( 'if not provided an input value, the accumulator function returns the current accumulated value (known mean, ignoring NaNs)', function test( t ) {
	...
	t.equal( acc(), sqrt( 2.0/3.0 ) / 2.0, 'returns expected value while ignoring NaNs' );
	t.end();
});
